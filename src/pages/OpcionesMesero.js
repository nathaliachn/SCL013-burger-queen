...
export default function Opciones() {
  ...
  const [selectedItems, setSelectedItems] = useState([]);  const handleItemClick = item => {
      ...
      <div className="App-menu">
        ...
        {data[menuType].map(item => (
          <div className="App-menu__item" onClick={() => handleItemClick(item)}>
            {item.producto} <span>${item.precio}</span><img className="imagenPedido" src={item.img} alt={item.producto}/>
          </div>
        ))}
      </div>
      <div className="App-list">
        <div className="row">
          <div className="col-sm">
            <h1><i class="fas fa-concierge-bell"></i>Pedidos</h1>            {selectedItems &&
              selectedItems.map(item => (
                <div className="App-list__item">
                  {item.producto} <span>${item.precio}</span>
                  <button className= "buttonDelete" onClick={() => handleDeleteItem(item.id)}>Eliminar</button>
                </div>
              ))}
